import {consoleTimeit, inverseMatrix2, MATRIX, matrixVectorMultiply, POINT, pointAdd} from '../util';
...
function parse(input: string[]) {
  ...
  for (const line of input) {
    if (line.startsWith('Button A:')) currentMachine.buttonA = line.split(/[+,]/).filter((_, i) => i === 1 || i === 3).map((x) => +x) as POINT;
    else if (line.startsWith('Button B:')) currentMachine.buttonB = line.split(/[+,]/).filter((_, i) => i === 1 || i === 3).map((x) => +x) as POINT;
    else if (line.startsWith('Prize:')) {
      currentMachine.prize = line.split(/[=,]/).filter((_, i) => i === 1 || i === 3).map((x) => +x) as POINT;
...
const part = (offset: POINT) => (input: string[]) => {
  const machines = parse(input);

  let tokens = 0;
  for (const machine of machines) {
    const {buttonA: [ax, ay], buttonB: [bx, by], prize: inputPrize} = machine;
    const prize = pointAdd(inputPrize, offset);
    const matrix: MATRIX = [[ax, bx], [ay, by]];
    const inv = inverseMatrix2(matrix);
    if (!inv) {
      console.debug(`Matrix ${matrix} is not invertible`);
      continue;
    }
    const rr = matrixVectorMultiply(inv, prize);
    const r = rr.map(x => Math.round(x));
    if (r[0] < 0 || r[1] < 0) {
      console.debug(`Matrix ${matrix} has no positive solution ${rr}`);
      continue
    }
    const [cx, cy] = matrixVectorMultiply(matrix, r);
    if (cx !== prize[0] || cy !== prize[1]) {
      console.debug(`Matrix ${matrix} has no integer solution ${rr}`);
      continue;
    }
    tokens += Math.round(r[0]) * 3 + Math.round(r[1]);
  }
  return tokens;
};

const part1 = part([0, 0]);

const part2 = part([10000000000000, 10000000000000]);
