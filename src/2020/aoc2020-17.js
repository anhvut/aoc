...
const lines = fs.readFileSync('aoc2020-17.txt', 'utf-8').split(/\r?\n/);

const MAX_Z = 20;
const MAX_Y = lines.length + 20;
const MAX_X = lines[0].length + 20;

const map = Array(MAX_Z).fill(null).map(_ =>
    Array(MAX_Y).fill(null).map(__ =>
        Array(MAX_X).fill(0)
    )
);
...
const wmap = Array(MAX_W).fill(null).map(___ =>
    Array(MAX_Z).fill(null).map(_ =>
        Array(MAX_Y).fill(null).map(__ =>
            Array(MAX_X).fill(0)
        )
    )
...
const part2 = () => {
    ...
    return current.reduce((sw, xyz) => sw + xyz.reduce((sz, xy) => sz + xy.reduce((sy, xs) => sy + xs.reduce((sx, x) => sx + x, 0), 0), 0), 0);
}
