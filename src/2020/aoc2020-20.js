const fs = require('fs');
const lines = fs.readFileSync('aoc2020-20.txt', 'utf-8').split(/\r?\n/);
...
function *getBorders(tile, withFlip) {
...
const searchMatches = () => {
  ...
  const matches = [];
  const tileIds = Object.keys(tiles).map(x => +x).sort((a, b) => a - b);
  for (let i = 0; i < tileIds.length - 1; i++) {
    ...
    for (let j = i + 1; j < tileIds.length; j++) {
      ...
      if (match) matches.push([{tileId: tileId1, ...match.variant1}, {tileId: tileId2, ...match.variant2}]);
    }
  }
  return matches;
}

const tileIdCounts = (candidates) => {
  const counts = {};
...
function *getTileVariants(tile) {
  yield tile;
  yield tile.slice().reverse();
  let currentRotation = tile;
  const lastY = tile.length - 1;
  for (let i = 0; i < 3; i++) {
    const nextRotation = Array(currentRotation[0].length).fill(0).map((_, x) => currentRotation.map((line, y, a) => a[lastY - y][x]).join(''));
    yield nextRotation;
...
const pasteRight = (superTile, pasteTile, xPaste, yPaste) => {
  const startYIndex = yPaste*pasteTile.length;
  const lineXIndex = xPaste*pasteTile[0].length - 1;
  const line = superTile.slice(startYIndex, startYIndex + pasteTile.length).map(x => x[lineXIndex]).join('');
  let rotatedTile;
...
const pasteBottom = (superTile, pasteTile, xPaste, yPaste) => {
...
const part2 = () => {
  ...
  const needFlipHoriz = matchesFirstTile.flat().find(x => x.tileId === firstTileId && [1, 3].includes(x.borderId)).borderId === 3;
  const needFlipVert = matchesFirstTile.flat().find(x => x.tileId === firstTileId && [0, 2].includes(x.borderId)).borderId === 0;
  ...
  const rightTileId = matches.find(x => x.some(y => y.tileId === firstTileId && [1, 3].includes(y.borderId))).flat().find(x => x.tileId !== firstTileId).tileId;
  const bottomTileId = matches.find(x => x.some(y => y.tileId === firstTileId && [0, 2].includes(y.borderId))).flat().find(x => x.tileId !== firstTileId).tileId;
  const alreadyFoundId = {
    [firstTileId]: true,
    [rightTileId]: true,
    [bottomTileId]: true
  };
  const rightBottomMatches = matches.filter(x => x.some(y => y.tileId === rightTileId || y.tileId === bottomTileId));
  const rightBottomCounts = tileIdCounts(rightBottomMatches.flat());
  const rightBottomTileId = +Object.entries(rightBottomCounts).find(([id, counts]) => counts === 2 && !alreadyFoundId[id])[0];
  ...
  const tileMap = [[firstTileId, rightTileId], [bottomTileId, rightBottomTileId]];
  const puzzleSize = Math.sqrt(Object.values(tiles).length);

  const complete = (x, y) => {
    const lastId = tileMap[y]?.[x - 1];
    const lastId2 = tileMap[y - 1]?.[x];
    const foundExceptLast = {...alreadyFoundId, ...(lastId && {[lastId]: false}), ...(lastId2 && {[lastId2]: false})};
    const candidates = matches.filter(a => a.some(b => [lastId, lastId2].includes(b.tileId)) && !a.some(b => foundExceptLast[b.tileId])).flat().filter(a => !alreadyFoundId[a.tileId]);
  ...
  for (let y = 0; y < monster.length; y++)
    for (let x = 0; x < monster[0].length; x++)
      if (monster[y][x] === '#') monsterPos.push([x, y]);
  let maxMonsters = 0;
  for (const rotatedTile of getTileVariants(tileNoBorder)) {
    ...
    for (let ry = 0; ry < rotatedTile.length - monster.length + 1; ry++) {
      for (let rx = 0; rx < rotatedTile[0].length - monster[0].length; rx++) {
        nbMonster += monsterPos.every(([x, y]) => rotatedTile[ry + y][rx + x] === '#');
      }
    }
    maxMonsters = Math.max(maxMonsters, nbMonster);
  }
  return tileNoBorder.reduce((a, l) => a + Array.from(l).reduce((b, x) => b + (x === '#'), 0), 0) - maxMonsters * monsterPos.length;
}
