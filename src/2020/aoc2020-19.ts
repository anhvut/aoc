...
const data = lines.reduce<DATA>(
  ({ state, candidates, rules }, line) => {
    if (line === '') {
...
const cache: string[][] = [];
const mix = (arr: string[][], n: number, prefix: string, acc: string[]): string[] => {
  if (n === arr.length - 1) {
...
const fill = (n: number) => {
  ...
  if (Array.isArray(rule)) {
    result = rule.flatMap((r: number[]) => {
      const tokens: string[][] = r.map((rr) => fill(rr));
      const acc: string[] = [];
...
const part1 = () => {
  const valid = cache[0].reduce<Record<string, number>>((agg, a) => {
    agg[a] = 1;
    return agg;
  }, {});
...
const part2 = () => {
  const valid42 = cache[42].reduce<Record<string, number>>((agg, a) => {
    agg[a] = 1;
    return agg;
  }, {});
  const valid31 = cache[31].reduce<Record<string, number>>((agg, a) => {
    agg[a] = 1;
    return agg;
  }, {});
  ...
  const isValid = (c: string) => {
    ...
    const parts = Array(nbParts)
      .fill(0)
      .map((_, i) => c.slice(i * 8, (i + 1) * 8));
    let nb42 = 0,
      nb31 = 0;
