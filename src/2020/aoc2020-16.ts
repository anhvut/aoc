...
const data = lines.reduce<AGG>(
  ({ state, zones, ticket, nearby }, line) => {
    if (line === '') {
...
const part1 = () => {
  ...
  return data.nearby.reduce(
    (ss, nn) =>
      ss + nn.reduce((s, n) => s + (zones.some(([a, b, c, d]) => (a <= n && n <= b) || (c <= n && n <= d)) ? 0 : n), 0),
    0
  );
};
...
const part2 = () => {
  ...
  const nearby = data.nearby.filter((nn) =>
    nn.every((n) => zonesValues.some(([a, b, c, d]) => (a <= n && n <= b) || (c <= n && n <= d)))
  );
  const possibilities = nearby[0].map((_) => Object.fromEntries(zoneEntries.map(([k, _v]) => [k, true])));
  nearby.forEach((nn) =>
    nn.forEach((n, i) =>
      zoneEntries.forEach(([zone, [a, b, c, d]]) => {
        if (!((a <= n && n <= b) || (c <= n && n <= d))) {
