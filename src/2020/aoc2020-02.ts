...
function part1() {
  console.log(
    input.reduce((agg, i) => {
      ...
      const occur = Array.from(word).reduce((l, c) => l + Number(c === letter), 0);
      return agg + Number(min <= occur && occur <= max);
    }, 0)
  );
}
...
function part2() {
  console.log(
    input.reduce((agg, i) => {
      ...
      const letter = i.split(' ')[1][0];
      const word = i.split(': ')[1];
      return agg + Number((word[p1] === letter && word[p2] !== letter) || (word[p2] === letter && word[p1] !== letter));
    }, 0)
  );
}
