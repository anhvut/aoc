const part1 = (input: string[]) => {
  ...
  const isDigit = (char: string) => zeroCp <= char.codePointAt(0) && char.codePointAt(0) <= nineCp;
  return input
    .map((line) => {
      const chars = line.split('');
      const firstDigit = chars.find(isDigit).codePointAt(0) - zeroCp;
      const lastDigit = chars.findLast(isDigit).codePointAt(0) - zeroCp;
...
const part2 = (input: string[]) => {
  ...
  const decode = (str: string) => {
    ...
    if (zeroCp <= cp && cp <= nineCp) return cp - zeroCp;
    for (let i = 0; i < digits.length; i++) {
      if (str.startsWith(digits[i])) return i;
    }
    return null;
  };
