const zeroCp = '0'.codePointAt(0);
const nineCp = '9'.codePointAt(0);
const getDigit = (char: string) => {
  if (char == null) return null;
  const cp = char.codePointAt(0);
  return zeroCp <= cp && cp <= nineCp ? cp - zeroCp : null;
};

const isSymbol = (char: string) => char != null && char !== '.' && getDigit(char) == null;

type SerializedCoords = `${number}-${number}`;
...
const parse = (input: string[]) => {
  return input.flatMap((line, y) => {
    ...
    for (let x = 0; x < line.length; x++) {
      let digit;
      let nb = null;
      const symbols: Record<SerializedCoords, string> = {};
      while ((digit = getDigit(line[x])) != null) {
        for (let dy = -1; dy <= 1; dy++) {
          const sy = y + dy;
          for (let dx = -1; dx <= 1; dx++) {
            const sx = x + dx;
            const char = input[sy]?.[sx];
            if (isSymbol(char)) symbols[`${sx}-${sy}`] = char;
          }
        }
        nb = (nb ?? 0) * 10 + digit;
        x++;
...
const part1 = (input: string[]) => {
  ...
  return found.filter(({symbols}) => Object.keys(symbols).length > 0).reduce((a, {nb}) => a + nb, 0);
};

const part2 = (input: string[]) => {
  const found = parse(input);
  const numbersBySymbol: Record<SerializedCoords, number[]> = {};
  for (const {symbols, nb} of found) {
    for (const [c, symbol] of Object.entries(symbols)) {
      if (symbol === '*') {
        numbersBySymbol[c as SerializedCoords] = (numbersBySymbol[c as SerializedCoords] ?? []).concat(nb);
      }
    }
  }
  return Object.values(numbersBySymbol).reduce((a, nb) => (nb.length === 2 ? a + nb[0] * nb[1] : a), 0);
};
