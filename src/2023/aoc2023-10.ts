import {isClockwise, POINT, pointEquals, SERIALIZED_POINT, serializePoint} from "../util";

const parse = (input: string[]): string[][] => {
  return input.map((l) => Array.from(l));
};

const findStart = (input: string[][]): POINT => {
  for (let y = 0; y < input.length; y++) {
    for (let l = input[y], x = 0; x < l.length; x++) {
      if (l[x] === 'S') return [x, y];
    }
  }
  return null;
...
const findNeighbors = (m: string[][], [x, y]: POINT): Array<POINT> => {
  const r: Array<POINT> = [] as any;
  const test = (tx: number, ty: number, chars: string) => {
    if (m[ty] && chars.includes(m[ty][tx])) {
      r.push([tx, ty]);
    }
  };
  const c = m[y][x];
  if ('SL|J'.includes(c)) test(x, y - 1, 'F|7');
  if ('SF|7'.includes(c)) test(x, y + 1, 'L|J');
  if ('S7J-'.includes(c)) test(x - 1, y, 'FL-');
  if ('SFL-'.includes(c)) test(x + 1, y, '7J-');
  return r;
};
...
const part1 = (input: string[]) => {
  ...
  const visited: Record<SERIALIZED_POINT, boolean> = {[serializePoint(start)]: true};
  let current: Array<POINT> = [start];
  let distance = 0;
  while (true) {
    const next: Array<POINT> = current.flatMap<POINT>((pos) => findNeighbors(lines, pos)).filter((p) => !visited[serializePoint(p)]);
    if (!next.length) break;
    for (const p of next) visited[serializePoint(p)] = true;
    current = next;
    distance++;
  }
  return distance;
...
const part2 = (input: string[]) => {
  ...
  // find top-left and substitute S with corresponding char
  const start = findStart(lines);
  let topLeft = start;
  const [sx, sy] = start;
  const f = findNeighbors(lines, start);
  if (f.length !== 2) throw new Error('Invalid start');
  if (pointEquals(f[0], [sx, sy - 1]) && pointEquals(f[1], [sx, sy + 1])) lines[sy][sx] = '|';
  if (pointEquals(f[0], [sx - 1, sy]) && pointEquals(f[1], [sx + 1, sy])) lines[sy][sx] = '-';
  if (pointEquals(f[0], [sx, sy - 1]) && pointEquals(f[1], [sx - 1, sy])) lines[sy][sx] = 'J';
  if (pointEquals(f[0], [sx, sy - 1]) && pointEquals(f[1], [sx + 1, sy])) lines[sy][sx] = 'L';
  if (pointEquals(f[0], [sx, sy + 1]) && pointEquals(f[1], [sx - 1, sy])) lines[sy][sx] = '7';
  if (pointEquals(f[0], [sx, sy + 1]) && pointEquals(f[1], [sx + 1, sy])) lines[sy][sx] = 'F';
  const pathVisited: Record<SERIALIZED_POINT, boolean> = {[serializePoint(start)]: true};
  let pathToVisit: Array<POINT> = [start];
  while (true) {
    const next: Array<POINT> = pathToVisit.flatMap<POINT>((pos) => findNeighbors(lines, pos)).filter((p) => !pathVisited[serializePoint(p)]);
    if (!next.length) break;
    for (const p of next) {
      pathVisited[serializePoint(p)] = true;
      if (p[1] < topLeft[1] || (p[1] === topLeft[1] && p[0] < topLeft[0])) topLeft = p;
    }
    pathToVisit = next;
  }

  // follow path again and mark inside points
  const pathRevisited: Record<SERIALIZED_POINT, boolean> = {
    [serializePoint(topLeft)]: true,
    [serializePoint([topLeft[0], topLeft[1] + 1])]: true
  };
  let lastDirection: POINT = [0, -1];
  let currPoint = topLeft;
  let currentClockwise = true;
  const insideAdded: Record<SERIALIZED_POINT, boolean> = {};
  let insideToVisit: Array<POINT> = [];
  const tryAddInitialInsidePoint = (p: POINT) => {
    const serialized = serializePoint(p);
    if (pathVisited[serialized]) return;
    if (insideAdded[serialized]) return;
    insideToVisit.push(p);
    insideAdded[serialized] = true;
  }
  while (true) {
    const next: Array<POINT> = findNeighbors(lines, currPoint).filter((p) => !pathRevisited[serializePoint(p)]);
    if (next.length > 1) throw new Error(`Expected one point only from [${currPoint[0]}, ${currPoint[1]}]`);
    if (!next.length) break;
    for (const p of next) pathRevisited[serializePoint(p)] = true;
    const nextPoint = next[0];

    const nextDirection: POINT = [nextPoint[0] - currPoint[0], nextPoint[1] - currPoint[1]];
    const char = lines[currPoint[1]][currPoint[0]];
    if ('F7JL'.includes(char)) {
      currentClockwise = isClockwise(lastDirection, nextDirection);
      lastDirection = nextDirection;
    }
    const cornerUnit = currentClockwise ? 1 : -1;
    if (char === 'F') {
      tryAddInitialInsidePoint([currPoint[0] + cornerUnit, currPoint[1] + cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0], currPoint[1] + cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0] + cornerUnit, currPoint[1]]);
    }
    if (char === '-') tryAddInitialInsidePoint([currPoint[0], currPoint[1] + nextDirection[0]]);
    if (char === '7'){
      tryAddInitialInsidePoint([currPoint[0] - cornerUnit, currPoint[1] + cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0], currPoint[1] + cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0] - cornerUnit, currPoint[1]]);
    }
    if (char === '|') tryAddInitialInsidePoint([currPoint[0] - nextDirection[1], currPoint[1]]);
    if (char === 'J'){
      tryAddInitialInsidePoint([currPoint[0] - cornerUnit, currPoint[1] - cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0], currPoint[1] - cornerUnit]);
      tryAddInitialInsidePoint([currPoint[0] - cornerUnit, currPoint[1]]);
  ...
  // loop through inside points and fill surrounding area
  const insideVisited: Record<SERIALIZED_POINT, boolean> = {};
  while (true) {
    const nextToVisit: Array<POINT> = [];
    const tryAddToVisit = (p: POINT) => {
      const serialized = serializePoint(p);
      if (insideVisited[serialized]) return;
      if (pathVisited[serialized]) return;
      nextToVisit.push(p);
      insideVisited[serialized] = true;
    };
    for (const p of insideToVisit) {
      insideVisited[serializePoint(p)] = true;
    }
    for (const [x, y] of insideToVisit) {
      tryAddToVisit([x - 1, y]);
      tryAddToVisit([x + 1, y]);
      tryAddToVisit([x, y - 1]);
      tryAddToVisit([x, y + 1]);
    }
    if (nextToVisit.length === 0) break;
    insideToVisit = nextToVisit;
  }
  return Object.keys(insideVisited).length;
};

const runs = [
  1, 1,   // samples part 1
  1,
  1, 1, 1,  // samples part 2
  1  // 249 too low
];
...
if (runs[0]) console.log('part1 sample 1a', part1(inputSample1a));

if (runs[1]) console.log('part1 sample 1b', part1(inputSample1b));

if (runs[2]) {
  console.time('part1 real');
  console.log('part1 real', part1(inputReal));
  console.timeEnd('part1 real');
}

if (runs[3]) console.log('part2 sample 2a', part2(inputSample2a));

if (runs[4]) console.log('part2 sample 2b', part2(inputSample2b));

if (runs[5]) console.log('part2 sample 2c', part2(inputSample2c));
