export {};
const players0: [number, number] = [9, 4];

function* getDie(): Generator<number> {
  let value = 1;
  while (true) {
    yield value;
    value++;
    ...
    for (const [value, count] of quantumDice) {
      for (const w of universes) {
        ...
        if (score >= endScore) nbWin[nextPlayer] += newCount;
          ...
          nextUniverses.push({
            count: newCount,
            players: w.players.map((x, i) => (i === nextPlayer ? { position, score } : x)),
          });
      }
    }
  ...
  const solve = (
    ...
    for (const [value, count] of quantumDice) {
      ...
      if (nextScore >= endScore) wins[currentPlayer] += count;
        ...
        solve(1 - currentPlayer, otherPosition, nextPosition, otherScore, nextScore).forEach(
          (w, i) => (wins[i] += w * count)
        );
    }
    cache[key] = wins;
