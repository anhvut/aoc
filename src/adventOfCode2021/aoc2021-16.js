...
function toBinary(s) {
  return Array.from(s).map(x => ('000' + parseInt(x, 16).toString(2)).slice(-4)).join('');
}
...
function decodePacket(input) {
  ...
  if (t === 4) { // literal
    ...
    if (i === 0) {  // next 15 bits = length of subPacket
      const L = parseInt(input.slice(7, 22), 2);
      let totalLength = 22 + L;
...
function applyOperator(packet) {
  switch (packet.t) {
    ...
    case 5:
      return applyOperator(packet.subs[0]) > applyOperator(packet.subs[1]);
    case 6:
      return applyOperator(packet.subs[0]) < applyOperator(packet.subs[1]);
    case 7:
      return applyOperator(packet.subs[0]) === applyOperator(packet.subs[1]);
