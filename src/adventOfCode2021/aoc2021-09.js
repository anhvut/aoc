...
const lines = fs.readFileSync(__dirname + '/aoc2021-09.txt', 'utf-8').split(/\r?\n/);
const input = lines.map(l => Array.from(l).map(x => +x));

const evaluateAround = (fct, n, x, y) => fct(n, x - 1, y) && fct(n, x + 1, y) && fct(n, x, y - 1) && fct(n, x, y + 1);
...
const part2 = () => {
  ...
  for (const [x0, y0] of findLowPoints()) {
    const marks = {};
    const mark = (x, y) => marks[`${x}_${y}`] = true;
    const marked = (x, y) => marks[`${x}_${y}`];
    mark(x0, y0);
    const isLower = (n, x, y) => {
      const m = input[y]?.[x];
      return m == null || marked(x, y) || n <= m;
    }
    let bfs = [[x0 - 1, y0], [x0 + 1, y0], [x0, y0 - 1], [x0, y0 + 1]];
    while (bfs.length > 0) {
      const nextBfs = [];
      const addToBfs = (x, y) => {
        const m = input[y]?.[x];
