...
const lines = fs.readFileSync('aoc2020-14.txt', 'utf-8').split(/\r?\n/);
...
const part1 = () => {
    const {mem} = lines.reduce(({mem, mask0, mask1}, line) => {
...
const pad0 = (input, length) => {
    let r = input;
    while (r.length < length) r = ['0', ...r];
    return  r;
}

const writeX = (mem, baseOffset, mask, value) => {
...
const part2 = () => {
    const {mem} = lines.reduce(({mem, mask0, mask1, maskX}, line) => {
        if (line.startsWith('mask = ')) {
            const maskRaw = Array.from(line.slice(7));
