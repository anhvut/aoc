...
function getOres(
  reactionsByChemical: Record<string, Reaction>,
  maxLevel: number,
  chemicalLevels: Record<string, number>
) {
  let recipe: QuantityChemical[] = reactionsByChemical[FUEL].input;
  for (let level = 1; level < maxLevel; level++) {
    const quantityByProducer: Record<string, number> = {};
    for (const { quantity, chemical } of recipe) {
      if (chemicalLevels[chemical] > level) {
        // do not replace now, it will be re-output by sibling chemicals
...
const part1 = (): number => {
  const reactionsByChemical: Record<string, Reaction> = Object.fromEntries(
    reactions.map((r) => [r.output.chemical, r])
  );
