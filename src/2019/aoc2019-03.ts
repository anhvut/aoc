...
const instructions: Instruction[][] = lines.map((x) =>
  x.split(',').map((x) => ({ direction: x[0], length: +x.slice(1) }))
);
...
const follow = (instructions: Instruction[]): Record<string, number> => {
  ...
  for (const instruction of instructions) {
    const [dx, dy] = { R: [1, 0], L: [-1, 0], U: [0, 1], D: [0, -1] }[instruction.direction] as [number, number];
...
function getIntersection<T>(a: Record<string, T>, b: Record<string, T>): string[] {
